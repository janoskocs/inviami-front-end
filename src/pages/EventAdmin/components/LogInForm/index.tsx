...
const LogInForm = ({ setUser }: LogInFormProps) => {
  const { eventLink } = useParams<{ eventLink: string }>();
  ...
  const validateForm = () => {
    setFormErrors({ eventLink: '', adminCode: '' });
    const errors = { eventLink: '', adminCode: '' };

    if (!formInput.eventLink) {
      errors.eventLink = 'Please enter the event link.';
    }
    if (!formInput.adminCode) {
      errors.adminCode = 'Please enter your event PIN.';
    }

    if (errors.eventLink || errors.adminCode) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFormErrors({
        eventLink: 'Something went wrong. Please contact us on Etsy.',
        adminCode: '',
      });
    }
  };
      ...
      {!noTokenAllowed && (
        <section className="mx-auto p-6 mb-6 w-full bg-white rounded-lg shadow sm:max-w-md">
          ...
          <p className="">
            ...
            close this browser tab. By continuing to use this service, you agree
            to the use of these cookies, or you may click here to decline them.
            ...
            <form className="space-y-4 md:space-y-4" onSubmit={handleSubmit}>
              ...
              <div>
                ...
                <input
                  ...
                  // required={true}
                  maxLength={4}
                  value={formInput.adminCode}
                  onChange={(e) =>
                    setFormInput({ ...formInput, adminCode: e.target.value })
                  }
                />
